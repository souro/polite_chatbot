...
train_src_encodings = tokenizer(train_df['src'].values.tolist(), truncation=True, padding=True, max_length=256)
train_trg_encodings = tokenizer(train_df['trg'].values.tolist(), truncation=True, padding=True, max_length=256)

dev_src_encodings = tokenizer(dev_df['src'].values.tolist(), truncation=True, padding=True, max_length=256)
dev_trg_encodings = tokenizer(dev_df['trg'].values.tolist(), truncation=True, padding=True, max_length=256)
...
for idx in range(50):
    ...
    test_src = tokenizer(test_df['src'].values.tolist()[idx], truncation=True, padding=True, return_tensors='pt')
    generated_ids = model.generate(test_src["input_ids"].cuda())

    print('pred:', tokenizer.batch_decode(generated_ids, skip_special_tokens=False)[0])
    print('\n')
...
test_ip = 'send me the text files.'
test_src = tokenizer(test_ip, truncation=True, padding=True, return_tensors='pt')
generated_ids = model.generate(test_src["input_ids"].cuda())
print('pred:', tokenizer.batch_decode(generated_ids, skip_special_tokens=False)[0])
...
test_ip = 'yes, go ahead and remove it.'
test_src = tokenizer(test_ip, truncation=True, padding=True, return_tensors='pt')
generated_ids = model.generate(test_src["input_ids"].cuda())
print('pred:', tokenizer.batch_decode(generated_ids, skip_special_tokens=False)[0])
...
test_ip = 'jon please use this resignation letter in lieu of the one sent on friday'
test_src = tokenizer(test_ip, truncation=True, padding=True, return_tensors='pt')
generated_ids = model.generate(test_src["input_ids"].cuda())
print('pred:', tokenizer.batch_decode(generated_ids, skip_special_tokens=False)[0])
