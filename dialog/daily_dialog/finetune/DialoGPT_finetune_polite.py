...
tokenizer = AutoTokenizer.from_pretrained("microsoft/DialoGPT-medium", eos_token='<|endoftext|>', pad_token='<|pad|>')
#tokenizer = AutoTokenizer.from_pretrained("DialoGPT_models", eos_token='<|endoftext|>', pad_token='<|pad|>')
...
# train_dataset = DialoGPTDataset(train_df['contexts'].values.tolist(), train_df['utterances'].values.tolist(), tokenizer, max_length=256)
# dev_dataset = DialoGPTDataset(dev_df['contexts'].values.tolist(), dev_df['utterances'].values.tolist(), tokenizer, max_length=256)

# To train with polite utterances
train_dataset = DialoGPTDataset(train_df['contexts'].values.tolist(), train_df['polite_utterances'].values.tolist(), tokenizer, max_length=256)
dev_dataset = DialoGPTDataset(dev_df['contexts'].values.tolist(), dev_df['polite_utterances'].values.tolist(), tokenizer, max_length=256)
...
#model = AutoModelForCausalLM.from_pretrained("DialoGPT_models")
...
for epoch_i in range(0, epochs):
    ...
    if(avg_val_loss<best_val_loss):
        ...
        # model.save_pretrained('DialoGPT_best_model_polite')
        # tokenizer.save_pretrained('DialoGPT_best_model_polite')
...
with open('DialoGPT_finetune_tag-gen_polite.pkl', 'wb') as f:
